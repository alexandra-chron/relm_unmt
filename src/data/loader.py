...
def check_data_params(params):
    ...
    # MLM eval steps
    mlm_eval_steps = [s.split('-') for s in params.mlm_eval_steps.split(',') if len(s) > 0]
    params.mlm_eval_steps = [(s[0], None) if len(s) == 1 else tuple(s) for s in mlm_eval_steps]
    assert len(params.mlm_eval_steps) == len(set(params.mlm_eval_steps))
    ...
    required_mono = set([l1 for l1, l2 in (params.mlm_steps + params.clm_steps  + params.mlm_eval_steps) if l2 is None] + params.ae_steps + params.bt_src_langs)
    params.mono_dataset = {
        lang: {
