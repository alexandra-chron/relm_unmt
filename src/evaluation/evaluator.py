...
class Evaluator(object):
    ...
    def evaluate_mlm(self, scores, data_set, lang1, lang2):
        ...
        # compute perplexity, loss and prediction accuracy
...
class Enc2DecodersEvaluator(Evaluator):
    ...
    def __init__(self, trainer, data, params):
        ...
        super().__init__(trainer, data, params)
        self.encoder = trainer.encoder
        self.decoder_en = trainer.decoder_en
        self.decoder_ne = trainer.decoder_ne
    ...
    def evaluate_mt(self, scores, data_set, lang1, lang2, eval_bleu):
        ...
        self.encoder.eval()
        self.decoder_en.eval()
        self.decoder_ne.eval()

        encoder = self.encoder.module if params.multi_gpu else self.encoder
        decoder_en = self.decoder_en.module if params.multi_gpu else self.decoder_en
        decoder_ne = self.decoder_ne.module if params.multi_gpu else self.decoder_ne
        ...
        for batch in self.get_iterator(data_set, lang1, lang2):
            ...
            # encode source sentence from lang 1 -> en or ne
            enc1, _ = encoder('fwd', x=x1, lengths=len1, langs=langs1, causal=False)
            enc1 = enc1.transpose(0, 1)
            enc1 = enc1.half() if params.fp16 else enc1
            ...
            # decode target sentence
            if lang1_id == 'en':
                dec2, _ = decoder_ne('fwd', x=x2, lengths=len2, langs=langs2, causal=True, src_enc=enc1, src_len=len1)
                word_scores, loss, _, _ = decoder_ne('predict', tensor=dec2, pred_mask=pred_mask, y=y, get_scores=True)

            elif lang1_id == 'ne':
                dec2, _ = decoder_en('fwd', x=x2, lengths=len2, langs=langs2, causal=True, src_enc=enc1, src_len=len1)
                word_scores, loss, _, _ = decoder_en('predict', tensor=dec2, pred_mask=pred_mask, y=y, get_scores=True)
            else:
                raise Exception
            # update stats
            n_words += y.size(0)
            ...
            if eval_bleu:
                ...
                if params.beam_size == 1:
                    if lang1_id == 'en':
                        generated, lengths = decoder_ne.generate(enc1, len1, lang2_id, max_len=max_len)
                    else:
                        generated, lengths = decoder_en.generate(enc1, len1, lang2_id, max_len=max_len)

                else:
                    if lang1_id == 'en':
                        generated, lengths = decoder_ne.generate_beam(
                            enc1, len1, lang2_id, beam_size=params.beam_size,
                            length_penalty=params.length_penalty,
                            early_stopping=params.early_stopping,
                            max_len=max_len
                        ...
                        generated, lengths = decoder_en.generate_beam(
                            enc1, len1, lang2_id, beam_size=params.beam_size,
                            length_penalty=params.length_penalty,
                            early_stopping=params.early_stopping,
                            max_len=max_len
